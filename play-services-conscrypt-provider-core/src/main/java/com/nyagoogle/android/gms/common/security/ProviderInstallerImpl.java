...
package com.nyagoogle.android.gms.common.security;
    ...
    public static void insertProvider(Context context) {
        ...
        try {
            ...
            synchronized (lock) {
                ...
                if (res == 1) {
                    Security.setProperty("ssl.SocketFactory.provider", "com.nyagoogle.android.gms.org.conscrypt.OpenSSLSocketFactoryImpl");
                    Security.setProperty("ssl.ServerSocketFactory.provider", "com.nyagoogle.android.gms.org.conscrypt.OpenSSLServerSocketFactoryImpl");

                    SSLContext sslContext = SSLContext.getInstance("Default");
                    SSLContext.setDefault(sslContext);
                    HttpsURLConnection.setDefaultSSLSocketFactory(sslContext.getSocketFactory());

                    Log.d(TAG, "Installed default security provider " + ProviderInstaller.PROVIDER_NAME);
                } else {
                    throw new SecurityException("Failed to install security provider " + ProviderInstaller.PROVIDER_NAME + ", result: " + res);
                }
            }
        } catch (Throwable e) {
            Log.w(TAG, e);
